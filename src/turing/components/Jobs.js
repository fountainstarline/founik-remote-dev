...
const Jobs = () => {
    ...
    return (
        <div className="all-jobs grid lg:grid-cols-6 grid-cols-3 lg:gap-5 gap-3">
            <div onClick={handleJob} data-job="Not Sure" className={`bg-white cursor-pointer flex flex-col text-center items-center space-y-4 justify-center h-24 p-4 shadow-sm hover:shadow-lg rounded-sm ${jobs.includes('Not Sure') ? 'job-active': null} `}>
            ...
            {skills.map(({title, icon},index) => {
                return(
                    <div key={index} onClick={handleJob} data-job={title} className={`
                        bg-white cursor-pointer flex flex-col text-center items-center 
                        space-y-4 justify-center h-24 p-4 shadow-sm hover:shadow-lg rounded-md
